...
import { socket } from "../pages/MainLobby";
...
const JoinLobby: React.FC = () => {
  ...
  useEffect(() => {
    socket.on(SocketResponseType.LOBBY_ERROR_INCORRECT_USERNAME, () =>
      setIncorrectUsername(true)
    );
    socket.on(SocketResponseType.LOBBY_ERROR_NOT_FOUND, () =>
      setLobbyNotFound(true)
    );
    socket.on(SocketResponseType.LOBBY_ERROR_GAME_ONGOING, () =>
      setGameOngoing(true)
    );
  }, []);
    ...
    <main onKeyDown={(e) => e.key === "Enter" && submit()}>
      <Container>
        ...
        <div className="flex flex-col gap-2">
          <Button
            text={lobbyId.length === 0 ? "CREATE LOBBY" : "JOIN"}
            onClick={() => submit()}
          />
