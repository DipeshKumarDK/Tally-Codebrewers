...
export default function Navbar() {
  ...
  const handleClick = async () => {
    dispatch(changeSearchId(searchRef && searchRef.current!.value));
    navigate("/profile");
  };
        ...
        <section className="flex flex-grow justify-between h-full w-full pl-6 pr-6">
          <div className="flex flex-grow border-[1px] border-slate-100 ml-5 rounded-l-full rounded-r-full w-fit pr-2 mt-1 mb-1">
            ...
            <div className="flex flex-col justify-center">
              <FiSearch
                onClick={handleClick}
                className="h-8 w-8 p-1 mt-1 mb-1 bg-white text-slate-800 rounded-full"
              />
            </div>
          </div>
          <Link to="/lobby" className="flex justify-end ml-5">
            <h4 className="text-sm ml-5 font-semibold pt-5 pb-5 cursor-pointer">
              Lobby
            </h4>
          </Link>
          <Link to="/profile" className="flex justify-end ml-5">
            <h4 className="text-sm ml-5 font-semibold pt-5 pb-5 cursor-pointer">
              Profile
            </h4>
          </Link>
          <Link to="/" className="flex justify-end ml-5">
            <h4 className="text-sm ml-5 font-semibold pt-5 pb-5 cursor-pointer">
              Login
            </h4>
          </Link>
